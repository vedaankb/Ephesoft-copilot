...
from server.openclaw_client import Resolver
...
class FakeChannel:
    ...
    async def fill_row(self, row_selector: str, values):
        self.events.append(("fill_row", {"row_selector": row_selector, "values": values}))
        self.rows.append({
            "description": values[0] if len(values) > 0 else "",
            "qty": values[1] if len(values) > 1 else "",
            "unit_cost": values[2] if len(values) > 2 else "",
        })
        return len(values)

    async def exists(self, selector: str) -> int:
        self.events.append(("exists", {"selector": selector}))
        return 1  # resolved selectors are always "present" in the fake DOM

    async def active_tab_url(self) -> str:
        return "about:blank"


class FakeOpenClaw(Resolver):
    """Returns deterministic selectors based on the natural language description.

    Inherits ``resolve_verified`` from Resolver, so it exercises the real
    verification path (resolve -> channel.exists -> optional re-ask)."""
...
class TestFieldOps(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_fill_field_strips_currency_and_commas_for_amounts(self):
        ...
        self.assertEqual(channel.values["#invoice_date"], "2024-03-15")  # normalized to ISO
...
class TestTableOps(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_clear_then_insert_row_strips_currency(self):
        ...
        # New deterministic path fills the row in one fill_row op.
        self.assertTrue(any(e[0] == "fill_row" for e in channel.events))
    ...
    async def test_insert_row_falls_back_when_fill_row_unavailable(self):
        """If the extension has no fill_row, per-cell resolution still fills the row."""
        class NoRowChannel(FakeChannel):
            # Shadow fill_row with a property that raises, so hasattr(...) is False
            # (simulates an older extension without the fill_row command).
            @property
            def fill_row(self):
                raise AttributeError("no fill_row")

        channel = NoRowChannel()
        openclaw = FakeOpenClaw()
        await execute(
            Action(
                name=ActionName.INSERT_TABLE_ROW,
                parameters={"description": "Vaccine", "qty": "2", "unit_cost": "$30.00"},
            ),
            channel,
            openclaw,
        )
        self.assertEqual(channel.values[".row-description"], "Vaccine")
        self.assertEqual(channel.values[".row-qty"], "2")
        self.assertEqual(channel.values[".row-unit_cost"], "30.00")


class TestDateNormalization(unittest.IsolatedAsyncioTestCase):

    def test_normalize_date_common_formats(self):
        self.assertEqual(_normalize_date("03/15/2024"), "2024-03-15")
        self.assertEqual(_normalize_date("2024-03-15"), "2024-03-15")
        self.assertEqual(_normalize_date("3/5/24"), "2024-03-05")
        self.assertEqual(_normalize_date("Mar 15, 2024"), "2024-03-15")

    def test_normalize_date_leaves_unparseable(self):
        self.assertEqual(_normalize_date("not a date"), "not a date")
        self.assertEqual(_normalize_date(""), "")

    async def test_fill_field_normalizes_invoice_date(self):
        channel = FakeChannel()
        openclaw = FakeOpenClaw()
        action = Action(name=ActionName.FILL_FIELD,
                        parameters={"field_name": "invoice_date", "value": "03/15/2024"})
        await execute(action, channel, openclaw)
        self.assertEqual(channel.values["#invoice_date"], "2024-03-15")


class TestResolveVerification(unittest.IsolatedAsyncioTestCase):

    async def test_resolve_verified_reasks_when_no_match(self):
        """If the first selector matches nothing, resolver re-asks once."""
        calls = []

        class FlakyResolver(Resolver):
            async def resolve(self, description, page_html):
                calls.append(description)
                # First call returns a bad selector, second returns a good one.
                return "#bad" if len(calls) == 1 else "#good"

        class MissChannel(FakeChannel):
            async def exists(self, selector):
                self.events.append(("exists", {"selector": selector}))
                return 0 if selector == "#bad" else 1

        ch = MissChannel()
        r = FlakyResolver()
        sel = await r.resolve_verified("input field for x", "<html></html>", channel=ch)
        self.assertEqual(sel, "#good")
        self.assertEqual(len(calls), 2)  # re-asked exactly once
