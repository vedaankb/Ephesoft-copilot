...
async def _set_document_type(action, channel, openclaw) -> Dict[str, Any]:
    ...
    selector = await openclaw.resolve_verified(
...
def _normalize_date(value: str) -> str:
    """Best-effort normalize a date string to ISO YYYY-MM-DD; leave as-is if unparseable."""
    s = (value or "").strip()
    if not s:
        return s
    for fmt in ("%Y-%m-%d", "%m/%d/%Y", "%m-%d-%Y", "%m/%d/%y", "%d/%m/%Y", "%b %d, %Y", "%B %d, %Y"):
        try:
...
async def _insert_table_row(action, channel, openclaw) -> Dict[str, Any]:
    """Add one line item row, then fill it in a single deterministic page op."""
    description = action.parameters["description"]
    qty = action.parameters["qty"]
    unit_cost = action.parameters["unit_cost"].replace("$", "").replace(",", "").strip()

    html = await channel.get_html()
    insert_btn_selector = await openclaw.resolve_verified(
    ...
    # Pause for the new row to mount (slower in VMs).
    await asyncio.sleep(0.5)

    html = await channel.get_html()
    row_selector = await openclaw.resolve_verified(
        description=(
            "the most recently added (last) line item row container in the line items table "
            "— the <tr> or row element that was just inserted and is still empty"
        ),
        page_html=html,
        channel=channel,
    )
    action.selector_used = f"{insert_btn_selector} -> {row_selector}"

    # One page operation fills description/qty/unit_cost by input order in the row.
    filled = 0
    if hasattr(channel, "fill_row"):
        try:
            filled = await channel.fill_row(row_selector, [description, qty, unit_cost])
        except Exception as e:
            logger.warning(f"fill_row failed ({e}); falling back to per-cell resolve")

    if not filled:
        # Fallback: resolve and fill each cell individually.
        desc_selector = await openclaw.resolve_verified(
            description="the most recently added empty description input in the line items table",
            page_html=html, channel=channel,
        )
        qty_selector = await openclaw.resolve_verified(
            description="the most recently added empty quantity input in the line items table",
            page_html=html, channel=channel,
        )
        cost_selector = await openclaw.resolve_verified(
