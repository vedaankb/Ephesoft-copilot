...
class Resolver:
    """
    Pluggable element-resolution interface.

    Any backend (the default Gemini-based OpenClawClient below, or a real
    OpenClaw integration later) implements ``resolve`` and gets
    ``resolve_verified`` for free. This lets resolution be swapped behind a
    stable interface without touching tools.py / agent.py.
    """

    async def resolve(self, description: str, page_html: str) -> str:
        raise NotImplementedError

    async def resolve_verified(self, description: str, page_html: str, channel=None) -> str:
        """
        Resolve a selector, then confirm it actually matches an element on the
        page via the extension. If it matches nothing, re-ask once with that
        feedback so a single bad guess doesn't fail the whole action.
        """
        selector = await self.resolve(description=description, page_html=page_html)
        if channel is None or not selector:
            return selector
        try:
            count = await channel.exists(selector)
        except Exception:
            # If we can't verify (older extension, transient error), trust it.
            return selector
        if count:
            return selector

        logger.warning(
            f"Resolved selector '{selector}' matched nothing; re-resolving '{description}'"
        )
        retry_desc = (
            f"{description}. The previous CSS selector '{selector}' matched NO element on the "
            "page. Return a different, valid CSS selector that exists in the HTML."
        )
        return await self.resolve(description=retry_desc, page_html=page_html)


class OpenClawClient(Resolver):
