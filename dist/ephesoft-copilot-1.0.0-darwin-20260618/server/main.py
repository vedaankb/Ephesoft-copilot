...
async def ws_panel(websocket: WebSocket):
    ...
    # Send the live extension status immediately so a freshly opened panel
    # reflects reality instead of showing a stale "disconnected" dot.
    await ws_update({
        "type": "extension_status",
        "connected": state["channel"].is_connected() if state["channel"] else False,
    })
